...
class CheckpointRewardWrapper(gym.RewardWrapper):
    """
    A wrapper that adds a reward for well-timed sliding tackles in defensive scenarios.
    """
    def __init__(self, env):
        super().__init__(env)
        self.sticky_actions_counter = np.zeros(10, dtype=int)
        self.tackle_success_reward = 1.0
        self.sliding_duration = 5
    ...
    def reward(self, reward):
        ...
        for rew_index in range(len(reward)):
            ...
            # Check if the controlled player has active sliding action
            if 'sticky_actions' in o and o['sticky_actions'][9] == 1:
                self.sticky_actions_counter[9] += 1
            else:
                self.sticky_actions_counter[9] = 0
            
            # Reward the tackler if the tackle has been held for a specific duration and it stops an opponent with the ball
            if self.sticky_actions_counter[9] == self.sliding_duration:
