...
def main(model, n_decomposition, n_reward, temperature, task, alg_cfg, use_doe, n_improve_iter):
    ...
    three_vs_one_with_keeper = f'{ROOT_DIR}/scenario_examples/{task}/academy_3_vs_1_with_keeper.py'
    corner = f'{ROOT_DIR}/scenario_examples/{task}/academy_corner.py'
    counterattack_easy = f'{ROOT_DIR}/scenario_examples/{task}/academy_counterattack_easy.py'
    counterattack_hard = f'{ROOT_DIR}/scenario_examples/{task}/academy_counterattack_hard.py'
    empty_goal = f'{ROOT_DIR}/scenario_examples/{task}/academy_empty_goal.py'
    empty_goal_close = f'{ROOT_DIR}/scenario_examples/{task}/academy_empty_goal_close.py'
    pass_and_shoot_with_keeper = f'{ROOT_DIR}/scenario_examples/{task}/academy_pass_and_shoot_with_keeper.py'
    run_pass_and_shoot_with_keeper = f'{ROOT_DIR}/scenario_examples/{task}/academy_run_pass_and_shoot_with_keeper.py'
    run_to_score = f'{ROOT_DIR}/scenario_examples/{task}/academy_run_to_score.py'
    run_to_score_with_keeper = f'{ROOT_DIR}/scenario_examples/{task}/academy_run_to_score_with_keeper.py'
    single_goal_versus_lazy = f'{ROOT_DIR}/scenario_examples/{task}/academy_single_goal_versus_lazy.py'
    five_vs_five = f'{ROOT_DIR}/scenario_examples/{task}/5_vs_5.py'
        ...
        for group_id in range(len(groups)):
            ...
            for iter in range(n_improve_iter):
                ...
                for response_r_id in range(n_reward):
                    ...
                    with open(
                            f"{REWARD_DIR}/reward_layer0_decomposition{response_id}_subtask{group_id}_iter{iter}_sample{response_r_id}.py",
                            'w') as file:
                    ...
                    with open(
                            f"{OUTPUT_DIR}/reward_layer0_decomposition{response_id}_subtask{group_id}_iter{iter}_sample{response_r_id}.py",
                            'w') as file:
                        file.writelines("import gym" + '\n')
                        file.writelines("import numpy as np" + '\n')
                        file.writelines(reward_code_string + '\n')
                    ...
                    with open(
                            f"{GFOOTBALL_DIR}/rewards/reward_layer0_decomposition{response_id}_subtask{group_id}_iter{iter}_sample{response_r_id}.py",
                            'w') as file:
                        file.writelines("import gym" + '\n')
                        file.writelines("import numpy as np" + '\n')
                        file.writelines(reward_code_string + '\n')
                    ...
                    create_task(CONFIG_ENVS_DIR, task, 0, response_id, response_r_id, group['number_of_agents'],
                                group['group_number'] - 1, iter, suffix)
                    create_train_cfg(CONFIG_ALGS_DIR, Time, alg_cfg, 0, response_id, response_r_id, group['number_of_agents'], group['group_number'] - 1, iter)
                    ...
                    rl_filepath = f"{OUTPUT_DIR}/{task}{suffix}_layer0_decomposition{response_id}_subtask{group_id}_iter{iter}_sample{response_r_id}.txt"
                    with open(rl_filepath, 'w') as f:
                        script_path = f'{SRC_DIR}/main.py'
                        params = [
                            'python', '-u', script_path,
                            f'--config={alg_cfg}_layer0_decomposition{response_id}_subtask{group_id}_iter{iter}_sample{response_r_id}',
                            f'--env-config={task}{suffix}_layer0_decomposition{response_id}_subtask{group_id}_iter{iter}_sample{response_r_id}',
                        ]
                ...
                for response_r_id, (code_run, rl_run) in enumerate(zip(code_runs, rl_runs)):
                    rl_run.communicate()
                    rl_filepath = f"{OUTPUT_DIR}/{task}{suffix}_layer0_decomposition{response_id}_subtask{group_id}_iter{iter}_sample{response_r_id}.txt"
                    result_path = f'{SACRED_DIR}/{Time}/scenario_layer0_decomposition{response_id}_subtask{group_id}/scoring, reward_layer0_decomposition{response_id}_subtask{group_id}_iter{iter}_sample{response_r_id}/1'
                    code_paths.append(
                        f"reward_layer0_decomposition{response_id}_subtask{group_id}_iter{iter}_sample{response_r_id}.py")
                    try:
                        ...
                        content += code_output_tip_rewards.format(number_of_agents=group['number_of_agents'],
                                                                          example_of_o=example_of_o, reward_signature=reward_signature)
                    ...
                    if traceback_msg == '':
                        if num_done_string < 6:
                            ...
                            score_reward_mean.append(DUMMY_FAILURE)
                            content += execution_error_feedback.format(traceback_msg="Wrong Scenario without Goalkeeper")
                        else:
                ...
                logging.info(f"Iteration {iter}: GPT Output Content:\n" + responses_r[best_sample_idx].message.content + "\n")
