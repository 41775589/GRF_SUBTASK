...
def train_merge_team(groups,
    ...
    if is_doe:
        if layer == "target":
            ...
            print(f'--config= doe_ippo_ns_layertarget_decomposition0_subtasktarget_itertarget_sampletarget_merged')
            print(f'--env-config=cookingzoo')
            with open(rl_logpath, 'w') as f:
                script_path = f'{SRC_DIR}/main.py'
                params = [
                    'python', '-u', script_path,
                    f'--config=doe_ippo_ns_zoo',
                    f'--env-config=cookingzoo',
                ]
...
if __name__ == "__main__":
    ...
    child_tasks = [
        {"group_number": 1, "number_of_agents": 1},
        {"group_number": 2, "number_of_agents": 1},
    ]

    # 原来是 group6 group7，感觉不太对劲，也许需要改成group 7 8？代表reward func
    max_reward_code_path_for_each_group={"group0": "reward_layer0_decomposition0_subtask0_iter0_sample0.py",
                                         "group1":"reward_layer0_decomposition0_subtask1_iter0_sample0.py"}

    # 指定本层merge的所有存储文件路径为：

    save_dir=f'{MEDoE_DIR}/doe_epymarl-main/results/zoo/{Time}/decomposition{response_id}/group{merge_group_id}'
