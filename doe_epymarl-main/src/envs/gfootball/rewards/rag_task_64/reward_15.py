...
class CheckpointRewardWrapper(gym.RewardWrapper):
    ...
    def reward(self, reward):
        ...
        for rew_index in range(len(reward)):
            ...
            # High cross completion
            if ('ball_owned_team' in o and o['ball_owned_team'] != -1 and 'game_mode' in o and
                o['game_mode'] == 4 and o['ball'][1] > 0.2):
                components["cross_reward"][rew_index] = self.cross_reward
            # Reward short passes to open spaces, promoting attack fluidity
            if ('ball_owned_team' in o and o['ball_owned_team'] != -1 and 'game_mode' in o and
                o['game_mode'] == 0 and o['ball'][0] > 0.5):
                components["pass_reward"][rew_index] = self.pass_reward
            
            reward[rew_index] += components["cross_reward"][rew_index] + components["pass_reward"][rew_index]
