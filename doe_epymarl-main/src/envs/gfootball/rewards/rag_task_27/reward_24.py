...
class CheckpointRewardWrapper(gym.RewardWrapper):
    ...
    def reward(self, reward):
        ...
        for rew_index in range(len(reward)):
            ...
            # Calculate distance from active player to ball
            distance_to_ball = np.linalg.norm(active_player_pos - ball_pos)
            
            # Reward for being close to the ball defensively (indicative of good positioning)
            if distance_to_ball < 0.1:
                components["defensive_positioning_reward"][rew_index] = 0.2

            # Check for potential interceptions or tackles
            ...
            reward[rew_index] += (components["defensive_positioning_reward"][rew_index] + components["interception_reward"][rew_index])
