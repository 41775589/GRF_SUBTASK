from . import *
def build_scenario(builder):
    builder.config().game_duration = 3000
    builder.config().right_team_difficulty = 0.05
    builder.config().left_team_difficulty = 0.05
    builder.config().deterministic = False

    # Configuring teams alternation for dynamic scenario complexity
    if builder.EpisodeNumber() % 2 == 0:
        first_team = Team.e_Left
        second_team = Team.e_Right
    else:
        first_team = Team.e_Right
        second_team = Team.e_Left

    builder.SetTeam(first_team)
    # Adding a goalkeeper who does not participate in field play
    builder.AddPlayer(-1.000000, 0.000000, e_PlayerRole_GK, controllable=False)
    # Adding the midfielder/defender hybrid player
    builder.AddPlayer(0.000000, 0.000000, e_PlayerRole_CM)

    builder.SetTeam(second_team)
    # Adding opposing goalkeeper
    builder.AddPlayer(-1.000000, 0.000000, e_PlayerRole_GK, controllable=False)
    # Adding a defensive line to challenge the agent, requiring use of long and high passes
    builder.AddPlayer(-0.5, 0.1, e_PlayerRole_CB)
    builder.AddPlayer(-0.5, -0.1, e_PlayerRole_CB)
    builder.AddPlayer(-0.5, 0.3, e_PlayerRole_CB)
    builder.AddPlayer(-0.5, -0.3, e_PlayerRole_CB)
